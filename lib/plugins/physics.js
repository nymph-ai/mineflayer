...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  const getAirBlock = () => {
    ...
    if (!bot.registry?.version || !bot.registry?.blocksByName?.air || !bot.registry?.blocks) return null
    const Block = require('prismarine-block')(bot.registry.version)
    airBlock = new Block(bot.registry.blocksByName.air.id, 0, 0)
    return airBlock
  }
  ...
  function tickPhysics (now) {
    if (world.getBlock(bot.entity.position) == null) return // check if chunk is unloaded
  ...
  bot._client.on('position', (packet) => {
    ...
    if (typeof packet.flags === 'object') {
      ...
      // Velocity uses dx/dy/dz fields; some servers send zeros without flags,
      // so only apply when velocity data is explicitly present or non-zero.
      const dx = packet.dx ?? 0
      const dy = packet.dy ?? 0
      const dz = packet.dz ?? 0
      if (packet.flags.dx || packet.flags.dy || packet.flags.dz || dx || dy || dz) {
        vel.set(
          packet.flags.dx ? (vel.x + dx) : dx,
          packet.flags.dy ? (vel.y + dy) : dy,
          packet.flags.dz ? (vel.z + dz) : dz
        )
      }
